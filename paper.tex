...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{figures/fig1_architecture_overview}
  \caption{Single-layer kill switch characterization. (A)~Escape rates for six architecturally distinct mechanisms calibrated from published experimental data. (B)~Mutation spectrum decomposition showing the contribution of point mutations, small indels ($<$50~bp), IS~element insertions, large deletions ($>$50~bp), and recombination events for each architecture. (C--D)~Escape takeover kinetics from stochastic serial passage simulations (10,000 replicates, $N_0 = 10^7$, $K = 10^9$, 1:100 dilution) with architecture-specific fitness costs calibrated from competitive index data.}
  \label{fig:arch}
\end{figure}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{figures/fig2_combination_heatmap}
  \caption{Multi-layer combination analysis. Heatmap of combined escape rates ($\log_{10}$, cell$^{-1}$ gen$^{-1}$) for all pairwise ($n = 10$) and triple ($n = 10$) combinations of five kill switch architectures at inter-layer correlation $\rho = 0.01$. The dashed line indicates the NIH-recommended threshold of $10^{-8}$. Green cells meet the threshold; red cells do not.}
  \label{fig:heatmap}
\end{figure}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{figures/fig3_correlation_sensitivity}
  \caption{Inter-layer correlation is the critical design parameter. (A)~Combined escape rate as a function of inter-layer correlation $\rho$ for representative combinations. The critical correlation $\rho_\text{crit} = 0.102$ at which the CRISPR multi-gRNA + overlapping gene combination loses NIH compliance is indicated by the vertical dashed line; the horizontal dashed line marks the NIH threshold. (B)~Two-dimensional phase diagram mapping the combined escape rate as a function of $\rho$ and the overlapping gene escape rate factor; contours show the safe operating region.}
  \label{fig:corr}
\end{figure}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{figures/fig4_pareto}
  \caption{Optimal design search. All 24 designs meeting the NIH threshold at $\rho = 0.01$ plotted as combined escape rate versus total fitness cost. Pareto-optimal designs (no other design achieves both a lower escape rate and lower fitness cost simultaneously) are highlighted; five designs form the Pareto front. The recommended triple-layer design (CRISPR multi-gRNA + overlapping gene + auxotrophy) is labeled.}
  \label{fig:pareto}
\end{figure}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{figures/fig5_validation}
  \caption{Framework validation. (A)~Leave-one-study-out cross-validation: predicted versus observed escape rates for five experimental measurements from two independent studies. The solid diagonal represents perfect prediction; dashed lines indicate one order of magnitude error. Mean absolute log-error $= 0.04$. (B)~Predicted and observed time to 10\% escape for the toxin-antitoxin and overlapping gene systems.}
  \label{fig:valid}
\end{figure}

\subsection{Out-of-Sample Prediction}

To test generalization beyond the calibration data, we applied the framework to predict escape rates for two independently published systems not used in parameter calibration: the STALEMATE entanglement system in \textit{E.~coli} Nissle\cite{Foo2025} and the \textit{thyA} auxotrophy + Cas9 device in \textit{B.~thetaiotaomicron}\cite{Hayashi2024}.

For the \textit{E.~coli} systems, three predictions were within the same organism class as our calibration data. The framework predicted the escape rate of the reoptimized entangled construct (pEnt-533) with near-perfect accuracy ($1.0 \times 10^{-7}$ predicted vs.\ $1.0 \times 10^{-7}$ observed; log error $= 0.00$). The non-entangled endonuclease (pEndo) was underpredicted by approximately one order of magnitude ($9.0 \times 10^{-7}$ predicted vs.\ $1.0 \times 10^{-5}$ observed), consistent with IS911 transposition rates being higher in \textit{E.~coli} Nissle than in the calibration strains. The entanglement + ColE9 policing system was overpredicted by 1.7 orders of magnitude, attributable to a population-level policing mechanism (released ColE9 kills escapers) not captured by our mutation-rate-based model. The mean absolute log error across fair comparisons was 0.92.

For the \textit{Bacteroides} system\cite{Hayashi2024}, predictions were 4--5 orders of magnitude below experimental observations, reflecting the fundamentally different biology of obligate anaerobic gut commensals compared to the \textit{E.~coli}/\textit{Pseudomonas} systems used for calibration. Specifically, \textit{thyA} auxotrophy in \textit{Bacteroides} is substantially leakier than in our model because thymidine can be scavenged from dead cells and the gut environment provides alternative nucleotide sources. This result demonstrates that the framework requires organism-specific calibration and should not be applied across phylogenetically distant species without recalibration.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{figures/fig6_ablation_sensitivity}
  \caption{Ablation and sensitivity analysis of the recommended triple-layer design (CRISPR multi-gRNA + overlapping gene + synthetic auxotrophy). (A)~Fold-increase in combined escape rate upon removal of each individual layer; error bars show bootstrap 95\% confidence intervals. (B)~Local sensitivity coefficients ($S$) quantifying the proportional change in combined escape rate per unit proportional change in each layer's individual escape rate.}
  \label{fig:ablation}
\end{figure}
...
\end{equation}
where $\rho_{ij}$ is the effective pairwise correlation between layers $i$ and $j$.

The physical motivation for Equation~\ref{eq:combination} is as follows. The first term represents independent escape: all $n$ layers must be simultaneously inactivated by independent mutations, giving the product of individual rates. The second term represents correlated co-escape: a single mutational event (e.g., a large genomic deletion spanning two adjacent loci, or an IS~element insertion disrupting a shared regulatory region) simultaneously inactivates layers $i$ and $j$, while the remaining layers must still be independently inactivated. The probability of such a co-escape event is proportional to $\rho_{ij} \cdot \max(\mu_i, \mu_j)$, where the $\max$ reflects that the co-escape rate is limited by the more vulnerable of the two layers (the mutation must affect the larger target). This formulation is analogous to a second-order inclusion-exclusion expansion for dependent events.

The effective correlation $\rho_{ij}$ is computed as:
\begin{equation}
\rho_{ij} = \rho_\text{base} + \rho_\text{IS}(i,j) + \rho_\text{del}(i,j) + \rho_\text{plasmid}
\label{eq:correlation}
\end{equation}
where $\rho_\text{base} \approx 0.01$ is a residual genomic correlation, $\rho_\text{IS}$ reflects shared IS~element susceptibility\cite{Consuegra2021}, $\rho_\text{del}$ reflects vulnerability to shared large deletions, and $\rho_\text{plasmid} \approx 0.15$ is added when components share a plasmid (plasmid loss eliminates both).
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_s1_convergence}
  \caption{Supplementary Figure~S1: Simulation convergence analysis. Standard error of the median time-to-escape as a function of replicate count (50--10,000 replicates). The standard error reaches 0.02~generations at $n = 10{,}000$ replicates, confirming that results are well-converged at the chosen replicate count.}
  \label{fig:s1}
\end{figure}

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_s2_deployment}
  \caption{Supplementary Figure~S2: Long-term deployment analysis. Cumulative escape probability over 5,000 generations (5,000 replicates) for the single CRISPR multi-gRNA system, the double CRISPR + auxotrophy combination, and the recommended triple-layer design. The triple design shows zero escape events through 5,000 generations, confirming indefinite containment within the simulation horizon.}
  \label{fig:s2}
\end{figure}

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_s3_environment}
  \caption{Supplementary Figure~S3: Growth environment sensitivity. Time to 50\% escape under five growth environments spanning rich laboratory media to nutrient-poor soil conditions. The 17-fold variation in escape dynamics (53--880~generations for the toxin-antitoxin system) demonstrates the importance of environment-specific calibration.}
  \label{fig:s3}
\end{figure}

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_s4_correlation_models}
  \caption{Supplementary Figure~S4: Correlation model sensitivity. Combined escape rates for the recommended triple-layer design under four alternative mathematical formulations of inter-layer correlation, as a function of $\rho$. The NIH threshold ($10^{-8}$) is indicated by the dashed horizontal line. The triple design maintains NIH compliance under two of four models at all tested correlation values including $\rho = 1$.}
  \label{fig:s4}
\end{figure}
