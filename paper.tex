...
Stochastic serial passage simulations (10,000 replicates, $N_0 = 10^7$, $K = 10^9$, 1:100 dilution) revealed the kinetics of escape takeover (Figure~\ref{fig:arch}C--D). The median time to 50\% escape ranged from 165~generations (single-gRNA CRISPR) to 792~generations (synthetic auxotrophy). Confidence intervals from 10,000 replicates were tight: standard error of the median was $<$0.4~generations for all architectures (see Supplementary Figure~S1).
...
The practical recommendation is the three-layer design of CRISPR multi-gRNA + overlapping gene + auxotrophy: at $6.06 \times 10^{-18}$ escape rate and 13\% fitness cost, it provides an enormous safety margin while maintaining reasonable metabolic burden. Adding a fourth layer (integrase differentiation) improves the rate to $9.62 \times 10^{-24}$ but increases the cost to 18\%, with marginal practical benefit.
...
For time-to-escape predictions, stochastic simulations overpredicted the number of generations to 10\% escape by approximately 6--8$\times$ compared to Chlebek~et~al.'s serial passage experiments (Figure~\ref{fig:valid}B). This discrepancy arises primarily from differences in effective population size: our simulations use $N_0 = 10^7$ (post-dilution), whereas the effective mutation supply population during exponential growth phases in the experiments may be substantially larger ($\sim$$10^9$). Additionally, the 37--51$\times$ competitive advantage of escape mutants measured by Chlebek~et~al.\cite{Chlebek2023} drives faster selective sweeps than captured by our model's per-generation fitness cost of 5\%.
...
Several limitations warrant discussion. First, escape rates are treated as fixed parameters, whereas they likely vary with growth conditions, host strain, and environmental context\cite{Rottinghaus2022}. Second, our stochastic model overpredicts time-to-escape by $\sim$6--8$\times$, likely due to population size and selective sweep dynamics. Third, we do not model horizontal gene transfer, which represents an additional containment failure mode addressed through separate strategies\cite{Hayashi2024}. Fourth, the correlation parameter $\rho$ is a scalar approximation of complex mutational dependencies that may vary by genomic context.
